...
import { formatImageUrl, PAGE_REG } from '@/lib/javbus-helper';
import { JAVBUS } from '@/constants/data';
import { getJavdbMagnetLinks } from './javdb-parser';
import { mergeMagnetArrays } from './magnet-helper';
...
function parseMoviesPage(pageHTML: string, filter?: (movie: Movie) => boolean): MoviesPage {
  ...
  const movies = doc
    ...
    .filter((movie): movie is Movie => Boolean(movie.id) && movie.date !== undefined)
