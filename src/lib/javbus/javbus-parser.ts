...
import { getAgent, getJavbusClient } from '@/lib/javbus/javbus-client';
import type {
  FilterType,
  GetMoviesQuery,
  ImageSize,
...
import { formatImageUrl, PAGE_REG } from '@/lib/javbus/javbus-helper';
import { JAVBUS } from '@/constants/data';
import { getJavdbMagnetLinks } from '@/lib/javdb/javdb-parser';
import { mergeMagnetArrays } from '@/lib/magnet/magnet-helper';
...
function parseMoviesPage(
  ...
  const movies = doc
    ...
    .map((item) => {
      const img =
        formatImageUrl(
          item.querySelector('.photo-frame img')?.getAttribute('src')
        ) ?? null;
      const title =
        item.querySelector('.photo-frame img')?.getAttribute('title') ?? '';
      const info = item.querySelectorAll('.photo-info date');
      const id = info[0]?.textContent;
      const date = info[1]?.textContent ?? null;
      const tags = item
        .querySelectorAll('.item-tag button')
        .map((item) => item.textContent);

      return { date, id, img, title, tags };
    })
    .filter(
      (movie): movie is Movie => Boolean(movie.id) && movie.date !== undefined
    )
    .filter((movie) => filter?.(movie) ?? true);

  const currentPage = Number(
    doc.querySelector('.pagination .active a')?.textContent ?? '1'
  );
  const pages = doc
...
export function parseStarInfo(pageHTML: string, starId: string): StarInfo {
  ...
  const avatar =
    formatImageUrl(
      doc?.querySelector('.photo-frame img')?.getAttribute('src')
    ) ?? null;
  ...
  const rest = mapKeys.reduce<Record<string, string | null>>((info, key) => {
    ...
    const value =
      infos
        ?.find((p) => p.textContent.includes(mapValue))
        ?.textContent.replace(mapValue, '') ?? null;
...
export function parseFilterInfo(
  pageHTML: string,
  type: FilterType,
  value: string
) {
...
export async function getMoviesByPage({
  ...
  const filterInfo =
    filterType && filterValue
      ? parseFilterInfo(res, filterType, filterValue)
      : undefined;
...
export function convertMagnetsHTML(html: string) {
  ...
  const magnets = doc
    ...
    .map<Magnet>((tr) => {
      ...
      const isHD = Boolean(
        tagAnchors?.find((a) => a.textContent.includes('高清'))
      );
      const hasSubtitle = Boolean(
        tagAnchors?.find((a) => a.textContent.includes('字幕'))
      );
      ...
      const size =
        tr.querySelector('td:nth-child(2) a')?.textContent.trim() ?? null;
      const numberSize = size ? bytes(size) : null;
      const shareDate =
        tr.querySelector('td:nth-child(3) a')?.textContent.trim() ?? null;
    ...
    .sort((a, b) =>
      a.numberSize && b.numberSize ? b.numberSize - a.numberSize : 0
    );
...
function linkInfoFinder(
  ...
  let id =
    link.getAttribute('href')?.replace(`${JAVBUS}/${computedPrefix}/`, '') ??
    '';
...
function multipleInfoFinder<T>(
  ...
  const infos =
    infoNodes
      .find(
        (info) => info.querySelectorAll('.genre').filter(genreFilter).length > 0
      )
...
export async function getMovieDetail(id: string): Promise<MovieDetail> {
  ...
  const img =
    formatImageUrl(
      doc.querySelector('.container .movie .bigImage img')?.getAttribute('src')
    ) ?? null;
  ...
  const similarMovies =
    doc
      ...
      .map((link) => {
        ...
        const img =
          formatImageUrl(link.querySelector('img')?.getAttribute('src')) ??
          null;

        return { id, title, img };
      })
      .filter(
        (movie): movie is SimilarMovie => Boolean(id) && Boolean(title)
      ) ?? [];
