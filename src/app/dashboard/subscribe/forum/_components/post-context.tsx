...
import { extractMagnetLinks } from '@/lib/magnet/magnet-helper';
import { useMemo, useState } from 'react';
import { toast } from 'sonner';
import { ForumPost } from '@prisma/client';
import { SingleThunderDownload } from '@/features/download/components/thunder-download';
const getValidMagnetLink = (text: string): string | null =>
  extractMagnetLinks(text.trim())[0] ?? null;
...
export function PostContext({
  ...
  const handleThunderDownload = () => {
    if (!selectedMagnetLink) return;
    SingleThunderDownload({ link: selectedMagnetLink });
  };
  const handleViewSourcePost = () => {
    // const forum = await
    window.open(
      getForumUrl(forum as string, post.postId, post.url || ''),
      '_blank',
      'noopener,noreferrer'
    );
  };
  ...
  return (
    <ContextMenu>
      ...
      >
        ...
        <div className=''>
          <div
            className='[&_a]:text-primary [&_blockquote]:border-primary [&_code]:bg-muted [&_pre]:bg-muted text-sm leading-relaxed [&_a]:underline [&_blockquote]:mb-4 [&_blockquote]:border-l-4 [&_blockquote]:pl-4 [&_blockquote]:italic [&_code]:rounded [&_code]:px-1 [&_code]:py-0.5 [&_h1]:mb-4 [&_h1]:text-2xl [&_h1]:font-bold [&_h2]:mb-3 [&_h2]:text-xl [&_h2]:font-bold [&_h3]:mb-2 [&_h3]:text-lg [&_h3]:font-semibold [&_img]:h-auto [&_img]:max-w-full [&_img]:rounded-lg [&_ol]:mb-4 [&_ol]:ml-6 [&_ol]:list-decimal [&_p]:mb-4 [&_pre]:mb-4 [&_pre]:overflow-x-auto [&_pre]:rounded-lg [&_pre]:p-4 [&_ul]:mb-4 [&_ul]:ml-6 [&_ul]:list-disc'
      ...
      <ContextMenuContent>
        ...
        {selectedMagnetLink && (
          <>
            ...
            <ContextMenuItem onClick={handleThunderDownload} inset>
              使用迅雷下载
            </ContextMenuItem>
          </>
        )}
