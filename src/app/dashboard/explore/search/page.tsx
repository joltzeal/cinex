  ...
  const filteredData = useMemo(() => {
    ...
    for (const source in searchResults.data) {
      ...
      const filteredList = sourceData.list.filter(item => {
        ...
        if (filters.minSize != null && itemSizeMB < filters.minSize) return false;
        if (filters.maxSize != null && itemSizeMB > filters.maxSize) return false;
        const fileCount = item.fileList ? parseInt(item.fileList.length.toString(), 10) : 0;
        if (!isNaN(fileCount)) {
          if (filters.minFiles != null && fileCount < filters.minFiles) return false;
          if (filters.maxFiles != null && fileCount > filters.maxFiles) return false;
        }
        return true;
      });
      ...
      <AnimatePresence mode="wait">
        {!hasSearched ? (
          ...
          <motion.div
            ...
            className="container mx-auto flex min-h-[70vh] items-center justify-center max-w-2xl p-4 md:p-8"
          >
            <Card className="w-full">
              <CardHeader>
                <CardTitle className="text-center text-2xl font-bold">资源搜索</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <motion.div
            ...
            exit="exit"
            transition={{ duration: 0.3 }}
            className="container mx-auto px-4 md:px-8 py-8 h-full" // Full width with padding
          >


            {error && (
              <div className="text-center text-red-500 p-8 bg-red-100 dark:bg-red-900/20 rounded-md">
