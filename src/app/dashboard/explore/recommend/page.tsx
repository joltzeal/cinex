      ...
      <div className="p-1.5 cursor-pointer" >
        <div className="group relative overflow-hidden rounded-lg">
          ...
          {['downloading', 'added', 'subscribed'].includes(video.status!) && (
            <div className="absolute bottom-2 right-2 flex items-center gap-1 rounded-md bg-black/60 px-2 py-0.5 text-xs font-semibold text-white backdrop-blur-sm">
              <Tag className="h-3 w-3" />
              <span>{SubscribeMovieStatusMap[video.status as keyof typeof SubscribeMovieStatusMap].label}</span>
            </div>
          )}
        </div>
      </div>
