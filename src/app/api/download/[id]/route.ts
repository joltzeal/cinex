...
export async function PUT(
  ...
  try {
    ...
    if (downloadURLsRaw) {
      ...
      // 获取当前文档中现有的URLs
      existingUrlsInDoc = existingDocument.downloadURLs.map(url => url.url);
      ...
      if (urlsToAdd.length > 0) {
        ...
        const existingUrlsInOtherDocs = await db.documentDownloadURL.findMany({
          where: {
            url: { in: urlsToCheck },
        ...
        if (existingUrlsInOtherDocs.length > 0) {
          const duplicateUrls = existingUrlsInOtherDocs.map(item => item.url);
          const errorMessages = duplicateUrls.map(duplicateUrl => {
