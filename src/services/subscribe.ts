...
export async function updateMoviesStatusByNumber(params: UpdateMoviesStatusByNumberParams) {
...
export async function updateMoviesDataByNumber(params: UpdateMoviesDataByNumberParams) {
...
export async function getSubscribeListWithMovies(whereClause: any, offset: number, pageLimit: number): Promise<Subscribe[]> {
  // 构建 Prisma 查询条件
  ...
  return await db.subscribe.findMany({
    where: prismaWhere,
    orderBy: {
      createdAt: 'desc'
    },
...
export async function getSubscribeListCount(
  ...
  return await db.subscribe.count({
    where: prismaWhere,
  });
}

export async function getSubscribeMovieCount(status: MovieStatus): Promise<number> {
...
export async function getSubscribeMovieList(params: GetSubscribeMovieListParams) {
...
export async function getWeeklyAddedMovieData() {
  const addedMovieList = await db.movie.findMany({
    where: {
      addedAt: {
        not: null,
  ...
  const sortedWeekDays = [...weekDays.slice(today), ...weekDays.slice(0, today)];
...
export async function getExistingSubscribe(filterType: string, filterValue: string) {
  return await db.subscribe.findFirst({
  where: { filterType, filterValue },
})}


export async function getExistingMovieStatusByNumber(number: string):Promise<MovieStatus | null>{
