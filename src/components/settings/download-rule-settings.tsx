...
const downloadRuleSchema = z.object({
  // 下载规则
  onlyChineseSubtitles: z.boolean().default(false),
  onlyHD: z.boolean().default(true),
  // 订阅规则
  checkForDuplicates: z.boolean().default(true),
  onlySingleMovie: z.boolean().default(true),
  downloadVR: z.boolean().default(false),
  downloadMagnetImmediately: z.boolean().default(true),
});
...
export function DownloadRuleSettings({ initialData }: DownloadRuleSettingsProps) {
  ...
  const form = useForm<DownloadRuleFormValues>({
    ...
    defaultValues: initialData || downloadRuleSchema.parse({}),
  });
