...
const scraperRuleSchema = z.object({
  // 命名规则
  fileNamingRule: z.string().min(1, '文件命名规则不能为空').default('number'),
  directoryRule: z.string().min(1, '目录规则不能为空').default('year/letters/[number]-actor'),
  nfoTitleRule: z.string().min(1, 'NFO 标题规则不能为空').default('number title'),
  ...
  // 水印类型 (存储一个字符串数组)
  watermarkTypes: z.array(z.string()).default(['subtitles', 'cracked', 'leaked', 'uncensored']),

  // 清理规则
  cleanupExtensions: z.string().optional().default('.html|.url'),
  cleanupFilenameContains: z.string().optional().default('直 播 盒 子|最 新 情 報|最 新 位 址|注册免费送|房间火爆|美女荷官|妹妹直播|精彩直播'),
  cleanupMinFileSize: z.coerce.number().min(0, '文件大小不能为负').default(100).optional(),
});
...
export function ScraperRuleSettings({ initialData }: ScraperRuleSettingsProps) {
  ...
  const form = useForm<ScraperRuleFormValues>({
    ...
    // 提供一个符合 schema 的默认结构
    defaultValues: initialData || scraperRuleSchema.parse({}),
  });
