...
export const MediaGrid = ({ movies, title }: { movies: Movie[], title: string }) => {
    ...
    <div className="flex flex-col gap-4 h-full">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Play size={20} />
          <h2 className="text-l font-bold tracking-tight">{title}</h2>
        </div>
        ...
        {movies.length === 0 ? (
          <div className="col-span-2 md:col-span-4 flex flex-col items-center justify-center py-16 rounded-xl bg-muted/50 border-2 border-dashed border-muted-foreground/20 ">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-xs text-muted-foreground/60">下载完成后的影片将显示在这里</p>
            </div>
          </div>
        ) : (
          movies.map((movie) => {
