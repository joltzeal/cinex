...
//     // {"Telegram": {"mediaType": "telegram", "downloadDir": "/download/telegram", "smartRename": true, "organizeMethod": "move", "scrapeMetadata": true, "mediaLibraryDir": "", "sendNotification": true}}
//     // 查找 mediaType 为 'telegram' 的配置
//     for (const key in obj) {
//       if (obj[key] && obj[key].mediaType === 'telegram') {
//         const downloadDir = obj[key].downloadDir;
//         if (downloadDir) {
//           return downloadDir; //"/download/telegram"
//         }
//       }
//     }
...
//           // 计算相对路径
//           const relativePath = path.relative(process.cwd(), 'media', filePath);
//           console.log(relativePath);

//           filePaths.push(path.join(DOWNLOAD_BASE_DIR, cleanChannelName, `album_${groupKey}`, fileName));
...
//         // 计算相对路径（相对于项目根目录）
//         // const relativePath = path.relative(process.cwd(), 'media', filePath);
//         const relativePath = path.join(DOWNLOAD_BASE_DIR, channelName, `message_${message.id}`, fileName);
...
//   const filePaths = [path.join(DOWNLOAD_BASE_DIR, channelName, `message_${message.id}`, path.basename(downloadedFile))];
